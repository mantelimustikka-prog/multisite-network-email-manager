...
class AdminMenu
{
    ...
    public function render_logs()
    {
        ...
        $sms_date_from        = '';
        $sms_date_to          = '';
        $sms_stats            = array('total' => 0, 'pending' => 0, 'sent' => 0, 'delivered' => 0, 'failed' => 0, 'rejected' => 0);
        ...
        if ($active_tab === 'sms') {
            ...
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            $sms_stats['rejected'] = (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(1) FROM {$queue_table} q WHERE {$sms_base_where} AND q.status = %s", 'rejected'));
    ...
    private function get_notice_message($notice)    {
        ...
        $messages = array(
            ...
            'queue_accounts_unsubscribed_deleted' => sprintf(
                '%d account%s unsubscribed and permanently deleted.%s',
                $count,
                $count === 1 ? '' : 's',
                $failed > 0 ? sprintf(' %d could not be processed.', $failed) : ''
            ),
            'queue_accounts_action_failed' => 'No accounts were unsubscribed or deleted. Please check the logs.',
