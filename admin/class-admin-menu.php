    ...
    public function render_queue()
    {
        ...
        if ($status_filter !== '') {
            ...
            $queue_items = (array) $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, blog_id, campaign_id, recipient_email, subject, status, attempts, scheduled_at, sent_at, opened, clicked, opens_count, clicks_count, created_at, provider_message_id, provider_metadata FROM {$queue_table} WHERE status = %s ORDER BY created_at DESC LIMIT %d OFFSET %d",
                    $status_filter,
                    $per_page,
                    $offset
                ),
        ...
        $this->render_view('queue.php', compact('queue_items', 'queue_stats', 'queue_summary', 'notice', 'notice_message', 'notice_class', 'total_all_records', 'total_filtered', 'current_page', 'total_pages', 'per_page', 'status_filter', 'all_statuses', 'offset'));
    }
