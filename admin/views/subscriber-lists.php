        ...
        <?php if ($active_list) : ?>
            ...
            <div class="mnem-panel mnem-panel-wide">
                ...
                <form method="get" action="<?php echo esc_url(network_admin_url('admin.php')); ?>" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
                    <input type="hidden" name="page" value="mnem-subscriber-lists" />
                    <input type="hidden" name="list_id" value="<?php echo esc_attr((string) $active_list['id']); ?>" />
                    <input type="hidden" name="subscribed_paged" value="1" />
                    <input type="hidden" name="unsubscribed_paged" value="1" />
                    <label for="mnem-subscriber-search"><strong>Search:</strong></label>
                    <input id="mnem-subscriber-search" type="search" name="subscriber_search" value="<?php echo esc_attr($subscriber_search); ?>" placeholder="Search username or email" />
                    <label for="mnem-subscriber-per-page"><strong>Per page:</strong></label>
                    <select id="mnem-subscriber-per-page" name="subscriber_per_page" onchange="this.form.submit();">
            ...
            <div class="mnem-panel mnem-panel-wide">
                ...
                <div style="margin-bottom: 15px;">
                    <a href="<?php echo esc_url(network_admin_url('admin.php?page=mnem-subscriber-lists-bulk-add&list_id=' . (int) $active_list['id'])); ?>" class="button button-primary">
                        <?php esc_html_e('+ Add from Network Users', 'multisite-network-email-manager'); ?>
                    </a>
                </div>
                <form method="post" action="<?php echo esc_url(network_admin_url('admin.php?page=mnem-subscriber-lists&list_id=' . (int) $active_list['id'])); ?>">
                    <?php wp_nonce_field('mnem_subscriber_lists'); ?>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button
                                    ...
                                    onclick="mnemOpenUnsubscribeModal(this);"
                                >Unsubscribe</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <div id="mnem-unsubscribe-modal" style="display:none;position:fixed;z-index:9999;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.5);">
                <div style="background:#fff;max-width:500px;margin:120px auto;padding:20px;">
                    <h3>Unsubscribe User</h3>
                    <p id="mnem-unsubscribe-user-label"></p>
                    <form method="post" action="<?php echo esc_url(network_admin_url('admin.php?page=mnem-subscriber-lists&list_id=' . (int) $active_list['id'])); ?>">
                        <?php wp_nonce_field('mnem_subscriber_lists'); ?>
                        <input type="hidden" name="mnem_action" value="subscriber_unsubscribe_user" />
                        <input type="hidden" name="list_id" value="<?php echo esc_attr((string) $active_list['id']); ?>" />
                        <input type="hidden" name="user_id" id="mnem-unsubscribe-user-id" value="" />
                        <label for="mnem-unsubscribe-reason">Reason (optional)</label>
                        <textarea id="mnem-unsubscribe-reason" name="unsubscribe_reason" rows="3" class="large-text" placeholder="Unsubscribed by admin"></textarea>
                        <div style="margin-top:10px;display:flex;gap:8px;">
                            <button type="submit" class="button button-primary">Confirm Unsubscribe</button>
                            <button type="button" class="button" onclick="mnemCloseUnsubscribeModal();">Cancel</button>
                        </div>
                    </form>
                </div>
            </div>
            <script>
                if (typeof mnemOpenUnsubscribeModal === 'undefined') {
                    function mnemOpenUnsubscribeModal(button) {
                        var modal = document.getElementById('mnem-unsubscribe-modal');
                        var userField = document.getElementById('mnem-unsubscribe-user-id');
                        var userLabel = document.getElementById('mnem-unsubscribe-user-label');
                ...
                if (typeof mnemCloseUnsubscribeModal === 'undefined') {
                    function mnemCloseUnsubscribeModal() {
                        var modal = document.getElementById('mnem-unsubscribe-modal');
                        if (!modal) {
                            return;
                        }
