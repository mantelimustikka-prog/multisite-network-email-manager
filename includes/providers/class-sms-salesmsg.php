...
/**
 * Salesmsg SMS Provider
 *
 * Requires:
 * - API Key
 *
 * Webhook Configuration:
 * Set the delivery webhook URL in Salesmsg dashboard under Settings > Integrations.
 *
 * Supported Statuses: sent, delivered, failed, undelivered
 */
class SmsSalesmsg extends SmsBaseProvider
{
    private const API_BASE = 'https://api.salesmsg.com/v1';
    ...
    public function test_connection(): array
    {
        ...
        if ($api_key === '') {
            return $this->error_result('Salesmsg API Key is required.');
        }
        ...
        if ($response['code'] === 200) {
            return $this->success_result('Salesmsg connected successfully.');
        }

        if ($response['code'] === 401) {
            return $this->error_result('Salesmsg authentication failed. Check your API Key.');
        }

        return $this->error_result('Salesmsg returned HTTP ' . $response['code'] . '.');
    }
    ...
    public function send(string $phone, string $message): array
    {
        ...
        if ($response['code'] === 200 || $response['code'] === 201) {
            ...
            return $this->success_result('SMS sent via Salesmsg.', $msg_id);
        }

        $detail = isset($data['message']) ? (string) $data['message'] : substr($response['body'], 0, 200);
        return $this->error_result('Salesmsg send failed (HTTP ' . $response['code'] . '): ' . $detail);
    }
